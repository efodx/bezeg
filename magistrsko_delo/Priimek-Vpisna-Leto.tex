...
\povzetek{V magistrskem delu obravnavamo Bezierjeve krivulje, racionalne Bezierjeve krivulje, zlepke Bezierjevih krivulj ter Bezierjeve PH krivulje.
Skozi delo predstavimo njihove lastnosti, ki so ključne za rabo v CAD/CAGD sistemih in algoritme, ki njihovo rabo v takšnih sistemih omogočajo.
...
%    nastopa, kje vse je bil obravnavan.
    ...
    \mycomment{Bezierjeve krivulje so marsikje uoprabljene za marsikaj in ph krivulje tudi..... }

%    Na koncu opišite tudi organizacijo dela -- kaj je v
%    katerem razdelku.
    Začeli bomo s predstavitvijo Bezierjevih krivulj, kjer bomo najprej spoznali Bernsteinove bazne polinome, na katerih Bezierjeve krivulje temeljijo.
    Predstavili bomo nekaj njihovih ključnih lastnosti, nato pa bomo z njihovo pomočjo upeljali Bezierjeve krivulje.
    Pokazali bomo nekaj osnovnih lastnosti krivulj, nadaljevali pa bomo z De Casteljaujevim algoritmom, ki je ključen za stabilno računanje točk Bezierjeve krivulje.
    Podrobneje si bomo ogledali lastnosti subdivizije, ekstrapolacije ter višanja stopnje Bezierjeve krivulje.
    Zaključili pa bomo z odvodi Bezierjeve krivulje, ki bodo pomembni pri tvorjenju zlepkov Bezierjevih krivulj v 6. poglavju.

    V tretjem poglavju se bomo posvečali racionalnim Bezierjevim krivuljam.
    Predstavili bomo delovanje uteži racionalne Bezierjeve krivulje in število dodatnih prostih parametrov, ki jih dobimo v primerjavi s polinomskimi Bezierjevimi krivuljami.
    Pokazali bomo racionalni De Casteljaujev algoritem, ki je razširitev algoritma iz prejšnjega poglavja.
    Nato pa bomo še pokazali Farinove točke, ki predstavljajo uporabniku CAGD sistema prijazno kontrolo nad utežmi.

    Četrto poglavje je namenjeno zlepkom Bezierjevih krivulj.
    Zlepke krivulj bomo najprej definirali, nato pa izpeljali pogoje gladkosti za zlepke Bezierjevih krivulj.
    Predstavili bomo koncept geometrijske zveznosti in izpeljali nekaj pogojev za različne stopnje geometrijske zveznosti.
    Proti koncu poglavja bomo podali nekaj algoritmov za konstrukcijo zlepkov Bezierjevih krivulj.
    Zaključili pa bomo z načinom računanja delilnih točk zlepka imenovanim $\alpha$- parametrizacije.

    V naslednjem poglavju predstavimo PH krivulje.
    Začnemo z motivacijo za njimi, kjer pokažemo, da nelinearne polinomske krivulje ne premorejo konstantne parametrične hitrosti.
    Kot alternativo predstavimo PH krivulje, ki imajo polinomsko parametrično hitrost.
    Nato s pomočjo Bernsteinovih polinomov izrazimo kontrolne točke Bezierjevih PH Krivulj kubične in kvintične stopnje.
    Izpeljemo parametrično hitrost Bezierjevih PH krivulj, ki jo nato uporabimo pri iskanju približka za enakomerno parametrizacijo.
    Pokažemo, da so funkcije tangente, normale in ukrivljenosti PH krivulje racionalne funkcije.
    Kot posledico izrazimo odmik krivuljo Bezierjeve PH krivulje kot racionalno Bezierjevo krivuljo.
    V zaključku poglavja izrazimo še kontrolne točke kubične in kvintične Bezierjeve PH krivulje s kompleksnimi števili, kar omogoča enostavno skaliranje in rotiranje krivulje.

    V šestem poglavju se posvetimo orodju za grafični prikaz lastnosti krivulj iz prejšnjih poglavij - Bezgu.
    Na začetku predstavimo uporabniški vmesnik orodja in njegove funkcionalnosti, ter čemu je orodje namenjeno.
    Drugi del poglavja pa je namenjen implementaciji orodja, kjer predstavimo arhitekturo aplikacije ter podamo nekaj primerov iz dejanske implementacije.
...
%
    ...
    Za $G^0$ mora veljati \[\mathbf{S_1}(u_1) =  \mathbf{S_2}(\varphi(u_1))=  \mathbf{S_2}(u_1),\] kar je enak pogoj, kakor za $C^0$.
    ...
    \[
        ...
        \end{bmatrix}  \begin{bmatrix}
                           ...
                           \mathbf{S_2''}(u_1)
        \end{bmatrix}.\]
    Lahko pa tudi najprej izberemo skalarje $\beta_i$ in konstruiramo reparametrizacijsko funkcijo, ki ustreza pogojem $\varphi'>0$,  $\varphi'(u_1)=\beta_1$ in $\varphi'(u_1)=\beta_2$.
    ...
    $G^1$ zlepek in vpliv izbire $\beta_1$ si lahko ogledamo na sliki~\ref{fig:zlepek-g1}.
    \begin{figure}[h]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
...
%    Pri slikovnem gradivu, bomo fiskne točke iz algoritmov označevali z zeleno barvo.
    ...
    Da je zlepek $C^r$ je dovolj, da krivulja $\mathbf{S}_2$ ustreza ekstrapolirani krivulji v prvih $r+1$ kontrolnih točkah, saj le te vplivajo na prvih $r$ odvodov.
    $C^r$ zlepek lahko torej konstruiramo tako, da začnemo z Bézierjevo krivuljo.
    ...
    \begin{algorithm}[H]
        ...
        \end{algorithmic}
    \end{algorithm}
    \noindent Zlepki konstruirani s takšnim algoritmom niso uporabniku prijazni, saj kontrolne točke različno močno vplivajo na obliko krivulje.
    ...
    Obnašanje tako konstruiranih zlepkov si lahko ogledamo na slikama~\ref{fig:simetricni-c1} in~\ref{fig:simetricni-c2}.
    \begin{algorithm}[H]
    ...
    \end{figure}
    Algoritem za simetrično konstrukcijo kvadratnega $C^1$ zlepka lahko adaptiramo tako, da upoštevamo pogoj za $G^1$ zveznost~\eqref{eq:g1-pogoj}.
    ...
    \end{figure}
    \subsubsection{$\alpha$ parametrizacije}
    ...
    Za točke $u_i$ bi torej želeli, da velja \[\frac{\Delta u_{i+1}}{\Delta u_i} = \frac{dolzina\_krivulje(\mathbf{s}^{(i+1)})}{dolzina\_krivulje(\mathbf{s}^{(i)})}.\]
...
%    Takšni hodografi definirajo regularne PH krivulje, saj za njih velja
    ...
    \begin{align}\label{eq:ph-bernstein-1}
    ...
    \begin{izrek}
        ...
        Naj bo $\mathbf{B}$ kubična Bézierjeva krivulja s kontrolnimi točkami $\p_0,\p_1,\p_2,\p_3$.
    ...
    \end{izrek}
    \noindent Pogoji iz izreka~\ref{izrek:geometrija-ph-poligona} so ekvivalentni temu, da si morata trikotnika $\triangle \p_0,\p_1,\p_2$ in $\triangle \p_1,\p_2,\p_3$ biti podobna, kar lahko vidimo na sliki~\ref{fig:geometrija-ph-poligona}.
    \begin{dokaz}
       \mycomment{Dodati moram dokaz, ali pa napisati, da ga ne bo.}
    \end{dokaz}
    \begin{figure}[h]
        \centering
    ...
    Če želimo podobno raven kontrole, kakor pri navadnih kubičnih Bézierjevih krivuljah, moramo poseči po \textit{kvintičnih} Bézierjevih PH krivuljah.
    Izpeljemo jih podobno, le da tokrat za polinoma $u$ in $v$  izberemo Bernsteinova polinoma polinoma stopnje $2$
    \begin{align}\label{eq:ph-bernstein-2}
    ...
    \begin{figure}[h]
        ...
        \caption{Kvintična Bézierjeva PH krivulja.}\label{fig:kvinticna-ph}
    \end{figure}
...
%    Izpeljimo še funkcijo dolžine PH krivulje.
    ...
    Dolžina izseka krivulje pri parametru $t \in [0,t_0]$, $t_0\in[0,1]$ je podana z določenim integralom
    ...
    Dobimo dolžino izseka Bézierjeve PH krivulje pri parametru $t \in [0,t_0]$ izraženo v Bernsteinovi bazi
    ...
    V razdelku~\ref{sec:konstantna-hitrost} smo pokazali, da polinomske krivulje stopnje višje od $1$ ne morejo imeti konstantne hitrosti.
    V tem razdelku bomo reševali problem, ki je s konstantno hitrostjo tesno povezan.
    Želimo najti takšne parametre $\{t_i\}_{i=0}^{N}$, $N\in \N$, da bodo izseki krivulje med njimi enako dolgi.
    ...
    \begin{figure}[h]
        ...
        \caption{$10$ točk na Bézierjevi PH krivulji pri ekvidistantnih $t_i$.}
    ...
    Znano je, da takšna iteracija pri začetnih približkih, ki so dovolj blizu parametra $t_k$, konvergira s kvadratno hitrostjo.
    ...
    \begin{figure}[h]
        ...
        \caption{$10$ točk na Bézierjevi PH krivulji, pridobljenih s parametri, za katere velja $s(t_{i+1})-s(t_i)= \frac{s(1)}{N}$.}
    ...
    Še več, odmik krivuljo Bézierjeve PH krivulje $\mathbf{r}$ lahko predstavimo kot racionalno Bézierjevo krivuljo s kontrolnimi točkami izraženimi s kontrolnimi točkami krivulje $\mathbf{r}$.
    ...
    Homogene koordinate kontrolnih točk racionalne odmik krivulje lahko v zaključeni obliki izrazimo s kontrolnimi točkami začetne Bézierjeve PH krivulje
    ...
    \mycomment{To ni očitno, moram dodati referenco na kak dokaz.}
...
%    Lahko dodam v program, da se narisejo te crtkane crte po katerih se premikajo kontrolne točke te odmik krivulje...}
    ...
    Kubične in kvintične Bézierjeve PH krivulje smo v prejšnjih podrazdelkih izpeljali tako, da smo izbrali Bernsteinove polinome definirane z izrazi~\eqref{eq:ph-bernstein-1},\eqref{eq:ph-bernstein-1} in integrirali.
    Če predstavimo konstante iz polinomov s kompleksnimi števili $\mathbf{w_i} = u_i +  iv_i\in \C$ in enako storimo s kontrolno točko $\p_0=x_0 + iy_0$, lahko preostale kontrolne točke kubične Bézierjeve PH krivulje izrazimo kot kompleksna števila z naslednjimi izrazi
    \[\p_1=\p_0+\frac{1}{3}\mathbf{w}_0^2,\quad \p_2=\p_1+\frac{1}{3}\mathbf{w}_0\mathbf{w}_1,\quad \p_3=\p_2+\frac{1}{3}\mathbf{w}_1^2.\]
    Podobno za kvintično Bézierjevo PH krivuljo dobimo izraze
    \begin{gather*}
    ...
    Zapišimo število $\mathbf{w_i}$ v polarnem zapisu, tj. $\mathbf{w_i}=r_i e^{i\varphi_i}$.
    Potem lahko rotacijo števila $\mathbf{w_i}$ okoli središča koordinatnega sistema za kot $\theta$ predstavimo z množenjem števila s kompleksnim številom $\mathbf{z}=e^{i\theta}$, tj.\ $\mathbf{z}\mathbf{w_i}=r_i e^{i(\varphi_i+\theta)}$.
    Kontrolne točke pri rotiranih točkah $\mathbf{\tilde{w}}_i=\mathbf{z}\mathbf{w}_i$ potem izledajo takole
    ...
    Od tod pa sledi, da rotiranje števil $\mathbf{w_i}$ okoli središča za kot $\theta$ rotira krivuljo okoli točke $\p_0$ za kot $2\theta$.
    Podobno dobimo, če za število $\mathbf{z}$ vzamemo skalar.
    V tem primeru skaliranje števil $\mathbf{w_i}$ za skalar $d$, skalira krivuljo za $d^2$.
    Primera rotacije in skaliranja krivulje s pripadajočimi števili $w_i$ si lahko ogledamo na sliki~\ref{fig:transformacije-ph-krivulje}.
    Več o kompleksni reprezentaciji Bézierjevih PH krivulj, si bralec lahko prebere v delu~\cite{farouki}.
    \begin{figure}[h]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
        \subfloat[štartna krivulja]{\includegraphics[width = \groupimagewidth]{images/ph-before-rotate}}\quad
        \subfloat[rotacija za $\pi$]{\includegraphics[width = \groupimagewidth]{images/ph-after-rotate}}\quad
        \subfloat[4x pomanjšanje v obe smeri]{\includegraphics[width = \groupimagewidth]{images/ph-after-scale}}
        \caption{Rotacija in skaliranje krivulje s pripadajočimi števili $w_i$.}
        \label{fig:transformacije-ph-krivulje}
    \end{figure}
    \section{Orodje za grafični prikaz lastnosti Bézierjevih in PH krivulj - \textit{Bezeg}}
    Vsi koncepti predstavljeni v magistrskem delu so implementirani v spletnem orodju Bezeg.
    Slikovno gradivo skozi magistrsko delo je bilo ustvarjeno s pomočjo Bezga.
    Orodje v celoti živi v uporabnikovem brskalniku in zato, razen za strežbo statične spletne strani, dodatnih zalednih sistemov ne potrebuje.
    Objavljeno je preko storitve GitHub Pages~\cite{github-pages}, ki omogoča zastonjsko objavo statične spletne strani.
    Spletno orodje pod krovom za izdelavo uporabniških vmesnikov uporablja knjižnico React,
    za risanje interaktivnih grafov knjižnico JSXGraph, za oblikovanje spletne strani pa uporablja ogrodje bootstrap v paru s knjižnico react-bootstrap.
    % Knjižnic ne bomo podrobneje opisovali, bralec si lahko o njih prebere na pripadajočih spletnih straneh.
    ...
    \item  Bézierjeva krivulja premore poleg osnovih ukazov še ukaze za subdivizijo, ekstrapolacijo, skrčitev, višanje stopnje, ter prikazovanje De Casteljaujeve sheme.
        ...
        \item Racionalna Bézierjeva Krivulja ima poleg funkcionalnosti Bézierjeve krivulje še dodane ukaze za prikaz in spreminjanje uteži, ter preklopnik za prikaz standardne forme uteži.
        \item PH Bézierjeve krivulje nimajo ukazov, ki so jih imele Bézierjeve krivulje, saj se pri nekaterih ukazih izgubi lastnost PH.
        Premorejo pa naslednje ukaze: prikazovanje odmik krivulj in spreminjanje velikosti odmika,
        prikaz kontrolnih točk odmik krivulj ter prikazovanje premic na katerih ležijo, dodajanje in odstranjevanje odmik krivulj, in koordinatni sistem z interaktivnimi točkami, ki predstavljajo hodografe.
        \item $C^n$ zlepki premorejo le ukaze osnovne krivulje, $G^1$ zlepki pa imajo poleg teh še drsnike za spreminjanje števil $\beta_1$.
    \end{itemize}
    \begin{figure}[h!]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
        ...
        \caption{Predogledi rezultatov različnih ukazov krivulje.}
    ...
    Zavoljo kompaknosti, bodo primeri kode v magistrskem delu pisani v mešanici psevdokode in TypeScripta.
    ...
    Razred JXGPointControlledCurve točke iz knjižnice JSXGraph ovije v ovoj JXGPointWrapper, ki nudi podporo za rabo točk v implementiranih krivuljah Bezga.
    Ovoj JXGPointWrapper je dvosmeren - spreminjanje točke v implementaciji krivulj spremeni točko iz knjižnice JSXGraph in obratno.
    Tako dosežemo to, da uporabnikovo premikanje točk iz knjižnice JSXGraph spreminja obliko krivulje, razne transformacije krivulje pa transformirajo tudi točke iz knjižnice JSXGraph.
    V naslednjih podrazdelkih bomo podrobneje predstavili posamezne člene Bezga.
    \begin{figure}[h!]
        \centering
...
interface PointControlledCurve {
 ...
 addPoint(point: Point): void;
    ...
    Za osnovno delo s krivuljami je na voljo razred PointImpl, ki predstavlja osnovno implementacijo funkcij iz vmesnika.
...
class PointImpl implements Point {
    ...
    \end{lstlisting}
    Druga implementacija vmesnika Point predstavlja ovoj za točke iz knjižnice JSXGraph, kjer metode le delegiramo na metode originalne točke iz knjižnice.
    Na primer
...
class JXGPointWrapper implements Point {
    ...
    \end{lstlisting}
    Takšen ovoj nam omogoča, da točke iz knjižnice JSXGraph uporabljamo pri konstrukciji svojih krivulj.

    \subsubsection{Implementacije krivulj}
    Osnovne implementacije vmesnika PointControlledCurve so združene v abstraktnem razredu AbstractPointControlledCurve, ki implementira vse metode razen calculatePointAtT\@.
    Metodo calculatePointAtT implementacije razreda AbstractPointControlledCurve implementirajo poljubno.
    Razredno hierarhijo krivulj v Bezgu si lahko ogledamo na sliki~\ref{fig:hiearhija-krivulj}.
    Za primer pokažimo kako je implementirana metoda affineTransform v razredu AbstractPointControlledCurve.
    ...
    \begin{figure}[h!]
        \centering
        \includegraphics[width = \textwidth]{drawio/classhierrhija.drawio}
...
interface BezierCurve extends PointControlledCurve {
    ...
    Razred PolynomialBezierCurve predstavlja implementacijo polinomske Bézierjeve krivulje.
...
class PolynomialBezierCurve extends AbstractPointControlledCurve implements BezierCurve {
 ...
 elevate(): PolynomialBezierCurve {
   ...
   return new PolynomialBezierCurve(newPoints);
    ...
    \end{lstlisting}
    Podobno implementiramo tudi razred, ki predstavlja racionalno Bézierjevo krivuljo RationalBezierCurve.
...
class RationalBezierCurve extends AbstractPointControlledCurve implements BezierCurve {
 ...
 extrapolate(t: number): this {
   ...
   return new RationalBezierCurve(newPoints, newWeights);
    ...
    Osnovno implementacijo zlepka Bézierjevih krivulj ponazarjamo z abstraktnim razredom BezierSpline.
    Razred ima polje, ki vsebuje seznam Bézierjevih krivulj ki ga sestavljajo.
    Bézierjeve krivulje generiramo s klicem abstraktne metode generateBezierCurves, ki jo implementacije zlepka implementirajo poljubno.
...
abstract class BezierSpline extends AbstractPointControlledCurve {
    ...
    Bézierjeve krivulje, ki ga definirajo konstruiramo po algoritmu~\ref{alg:C-n}.
    \begin{lstlisting}[label={lst:zlepek}]
class GenericBezierSpline extends BezierSpline {
 ...
 generateBezierCurves() {
   ...
   for (let i = this.degree; i<this.points.length-1; i = i+step) {
    ...
    Za delo s PH krivuljami je na voljo razred PhBezierCurve.
    Razred prejme kontrolno točko $\p_0$ in točke $\mathbf{w}_i$ predstavljene v razdelku~\ref{subsec:complex-repr}.
    Na podlagi števila vnešenih točk $\mathbf{w}_i$ se nato odloči katere izraze iz razdelka~\ref{subsec:complex-repr} uporabi pri generaciji točk polinomske Bezierjeve krivulje.
    Ker Bezierjeve PH krivulje niso afino invariantne, metodo affineTransform povozimo, da v primeru klica vrže napako.
    Rotacija, skaliranje krivulje v obe smeri in premik krivulje obdržijo PH lastnost, zato so v razredu implementirane metode, ki te funkcionalnosti podpirajo.
    Za primer si oglejmo konstruktor razreda, povoženo funkcijo affineTransform in funkciji moveFor ter scale.
    \begin{lstlisting}
export class PhBezierCurve extends AbstractPointControlledCurve {
  ...
  override affineTransform(A: number[][], b: number[], center: number[]): void {
        throw "PH Curves don't allow affine transformations.";
  });
...
\end{lstlisting}
    ...
    \subsubsection{Povezava implementacij krivulj s knjižnico JSXGraph}
    V Bezgu interaktivne Bézierjeve krivulje rišemo s pomočjo knjižnice JSXGraph.
    V razdelku~\ref{subsubsec:osnovni-vmesniki} smo povedali, da imamo na voljo razred, ki ovije točke iz knjižnice JSXGraph in omogoča njihovo rabo v naših krivuljah.
    Oglejmo si, kako lahko krivulje iz prejšnjega razdelka narišemo, in naredimo interaktivne, s knjižnico JSXGraph.
    \begin{lstlisting}[label={lst:ghdfg}]
    // Ustvarimo interaktivne tocke knjiznice jsxgraph
    ...
    \end{lstlisting}
    Na grafu se izriše Bézierjeva krivulja z izbranimi kontrolnimi točkami.
    Premik kontrolnih točk na grafu spreminja obliko krivulje.
    Za zgornje, in ostalo delo potrebno za izris grafa, v Bezgu poskrbijo implementacije razreda AbstractJXGPointControlledCurve, ki ima abstraktno metodo getInitialCurve.
   ...
   % Polinomsko Bézierjevo krivuljo lahko dodamo na graf JSXGraph knjižnice na naslednji način.
    \begin{figure}[h!]
        \centering
    ...
    \end{figure}
%    \begin{lstlisting}[label={lst:ghdfg}]
%        // board je instanca grafa iz knjiznice JSXGraph
...
%    \end{lstlisting}
    ...
    V naslednjem primeru naštejmo nekaj funkcionalnosti.
\begin{lstlisting}[label={lst:ghdfg}]
 ...
 jxgBezierCurve.moveFor(2,3);
    \end{lstlisting}
    Poleg takšnih metod ima razred JXGPointControlledCurve tudi metode za procesiranje potez uporabnikove miške processMouseDown, processMouseMove, processMouseUp, ki skrbijo za to, da lahko uporabnik z rabo miške upravlja z izbrano krivuljo.
 \subsubsection{Grafi}
    Za inicializacijo grafa knjižnice JSXGraph in funkcionalnosti naštete v razdelku~\ref{subsec:gui} skrbi abstrakten razred AbstractBaseGraph, ki ima abstraktno metodo initialize.
    ...
    \begin{figure}[h!]
        ...
        \label{fig:hiearhija-krivulj}
    \end{figure}
    Kot primer implementacije metode initialize si oglejmo kako graf racionalne Bezierjeve krivulje nariše svojo krivuljo
    \begin{lstlisting}[label={lst:ghdfg}]
 ...
 class ElevationGraph extends AbstractBaseCurveGraph {
    ...
    \end{lstlisting}
    Ukaze za kartico izbrane krivulje pa poda izbrana krivulja sama.
    Vse implementacije razreda JXGPointControlledCurve imajo namreč metodo getCurveCommands, ki vrača ukaze za kartico.
    Na primer razred JXGPHBezierCurve na naslednji način poda ukaze za upravljanje z odmik krivuljami
