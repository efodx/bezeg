...
%    Takšni polinomi so bili prvič uporabljeni v konstruktivnem(????) dokazu Weierstrassovega aproksimacijskega izreka.
    ...
    \begin{izrek}{Lastnosti Bernsteinovih polinomov}
        ...
        \begin{enumerate}
            ...
            \item{\label{izrek:bernsteinovi_lastnosti:mnozenje} Naj bosta $A_m$ in $B_n$ Bernsteinova polinoma stopnje $m$ in $n$ ter skalarji $\beta_i$ in $\alpha_i$ KAKO SE ZE RECE RESPECTIVELY....
    ...
    Da bo zlepek zvezen, mora v stični točki $u_1$ veljati $\mathbf{s}_1(1) = \mathbf{s}_2(0)$ oziroma $\p_n^{(1)} = \p_0^{(2)}$.
    Takšen zlepek, si lahko ogledamo na sliki ~\ref{fig:zlepek-c0}.
    Na sliki lahko vidimo, da zlepejk+
    \begin{figure}[h]
        \centering
    ...
    \begin{dokaz}
        DRUGAČE, referenciraj se na hodograf kvintične krivulje.... Odvajaj tisto.... bedno bo, tak je lajf.
    ...
    Tukaj lahko razpišem $\mathbf{O}_k$ za kvintične in kubične PH.
