...
%    Takšni polinomi so bili prvič uporabljeni v konstruktivnem(????) dokazu Weierstrassovega aproksimacijskega izreka.
    ...
    \begin{izrek}{Lastnosti Bernsteinovih polinomov}
        ...
        \begin{enumerate}
            ...
            \item $B_{i}^{n}'(t)=n(B_{i-1}^{n-1} - B_{i}^{n-1})$ \label{izrek:bernsteinovi_lastnosti:odvod}
    ...
    \begin{dokaz}
        ...
        \[B_i^n(1-t) = \binom{n}{i}(1-t)^i(1-(1-t))^{n-i} =  \binom{n}{n-i}(1-t)^it^{n-i} = B_^{n-i}_{i}(t).\]
    ...
    \begin{dokaz}
        ...
        \begin{align*}
            \phi\left(\sum_{i=0}^{n}\p_{i}B_i^n(t)(t)\right) &= A\left(\sum_{i=0}^{n}\p_{i}B_i^n(t)(t)\right) + \mathbf{b} &&=  \sum_{i=0}^{n}A\p_{i}B_i^n(t)(t) + \mathbf{b}  \\
