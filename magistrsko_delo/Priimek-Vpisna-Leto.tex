...
%
    ...
    \subsection{Konstrukcija zlepkov Bézierjevih krivulj}
...
%    Pri slikovnem gradivu, bomo fiskne točke iz algoritmov označevali z zeleno barvo.
    ...
    Če želimo upoštevati točki $d_{-1}$ in $d_{m+1}$, ju lahko upoštevamo pri izračunu $u_1$ in $u_m$ na naslednji način
    ...
    Izbira vpliva tako na obliko, kot tudi na enakomernost porazdeljenosti točk na krivulji, izračunanih pri ekvidistantnih $t_i$.
    \begin{figure}[h]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
...
%    Izpeljimo še funkcijo dolžine PH krivulje.
    ...
    Homogene koordinate odmik krivulje lahko v zaključeni obliki izrazimo s kontrolnimi točkami Bézierjeve PH krivulje
...
%    Lahko dodam v program, da se narisejo te crtkane crte po katerih se premikajo kontrolne točke te odmik krivulje...}
    ...
    \subsection{Reprezentacija s kompleksnimi števili}
    ...
    \subsection{Ukazi in transformacije izbrane krivulje}
...
interface PointControlledCurve {
 ...
 addPoint(point: Point): void;
    ...
    Za osnovno delo s krivuljami sem pripravil razred PointImpl.
...
class GenericBezierSpline extends BezierSpline {
 ...
 generateBezierCurves() {
   ...
   for (let i = this.degree; i<this.points.length-1; i = i+step) {
    ...
    S pomočjo knjižnice JSXGraph v Bezgu narišemo interaktivne Bézierjeve krivulje.
    ...
    Bézierjevo krivuljo lahko dodamo na graf JSXGraph knjižnice na naslednji način.
    \begin{lstlisting}[label={lst:ghdfg}]
 jxgBezierCurve = new JXGBezierCurve([[1,2],[2,3],[3,0]], board);
    \end{lstlisting}
    Na grafu se nariše Bézierjeva krivulja z izbranimi kontrolnimi točkami.
