...
Ogledamo si njegove funkcionalnosti in pokažemo osnovne ideje, kako je orodje implementirano in kako so v orodju predstavljene prej naštete krivulje.
}

% - povzetek v angleščini
\abstract{
    In the thesis, we discuss Bézier curves, rational Bézier curves, Bézier curve splines, and PH curves.
    Throughout the work, we present their properties, which are crucial for use in computer-aided design (CAD) and computer-aided geometric design (CAGD) systems, as well as algorithms that enable their use in such systems.
    We also introduce a tool for the graphical visualization of the properties of the aforementioned curves - Bezeg.
    We examine its functionalities and demonstrate the basic ideas of how the tool is implemented and how the aforementioned curves are represented in the tool.
}

% - klasifikacijske oznake, ločene z vejicami
%   Oznake, ki opisujejo področje dela, so dostopne na strani https://www.ams.org/msc/
\klasifikacija{65D17, 65D07, 68-04}

% - ključne besede, ki nastopajo v delu, ločene s \sep
\kljucnebesede{Bézierjeva krivulja\sep racionalna Bézierjeva krivulja\sep zlepek\sep krivulja s pitagorejskim hodografom\sep spletno orodje}

% - angleški prevod ključnih besed
\keywords{Bézier curve\sep rational Bézier curve\sep spline\sep pythagorean hodograph curve\sep web tool} % angleški prevod ključnih besed
...
\zahvala{
    ...
    Za vso podporo in zaupanje se zahvaljujem tudi partnerici, družini in prijateljem.
    Posebej bi izpostavil prijatelja Kristjana Bolčiča, ki mi je skozi študentsko življenje veliko pomagal tudi z deljenjem zapiskov in bil na voljo, ko se mi je s snovjo (oziroma v življenju) kje zataknilo.
}
...
%    Tako bomo večdimenzionalnost točke $x\in\R^{n+1}$ označili z $\mathbf{x}=(x_0,x_1,\dots,x_n)$, večdimenzionalnost funkcije $f:\R\to\R^{n+1}$ pa z $\mathbf{f}=\left( f_0,f_1,\dots,f_n\right)$.
    ...
    \begin{figure}[ht!]
        ...
        \subfloat[zrcaljenje čez abscisno os]{\includegraphics[width = \groupimagewidth]{images/afine-transform-flip}}
        \qquad
...
%    \end{dokaz}
    ...
    Ker so Bernsteinovi bazni polinomi definirani za vsa realna števila $t$, lahko Bézierjeve krivulje gledamo tudi izven domene $[0,1]$.
...
%&=\mathbf{p}_{0} + \sum_{i=1}^{n}\left(\mathbf{p}_{i}\frac{n+1-i}{n+1} + \mathbf{p}_{i-1}\frac{i}{n+1}\right)B_{i}^{n+1}(t)+\mathbf{p}_{n}.
    ...
    Opaziti je tudi možno, da so kontrolne točke z vsakim višanjem stopnje bližje začetni krivulji, kontrolni poligon pa se ji vedno bolj prilega.
    \begin{figure}[h]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
    ...
    Operator deluje na kontrolnih točkah $\p_i$, definiramo pa ga rekurzivno kot
...
%    Pri slikovnem gradivu, bomo fiskne točke iz algoritmov označevali z zeleno barvo.
    ...
    Za primer si na sliki~\ref{fig:enostranski-c1} oglejmo obnašanje tako konstruiranega kvadratičnega $C^1$ zlepka.
    \begin{figure}[h]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
        ...
        \caption{Obnašanje enostranskega kvadratičnega $C^1$ zlepka.}
    ...
    \begin{figure}[h]
        ...
        \caption{Obnašanje simetrično konstruiranega kvadratičnega $C^1$ zlepka.}
    ...
    \end{figure}
    Algoritem za simetrično konstrukcijo kvadratičnega $C^1$ zlepka lahko adaptiramo tako, da upoštevamo pogoj~\eqref{eq:g1-pogoj} za $G^1$ zveznost.
    ...
    \begin{figure}[h]
        ...
        \caption{Vpliv izbire $\beta_1$ pri simetričnem kvadratičnem $G^1$ zlepku.}
...
%    Lahko dodam v program, da se narisejo te crtkane crte po katerih se premikajo kontrolne točke te odmik krivulje...}
    ...
    \begin{figure}[h]
        ...
        \caption{Rotacija in skaliranje Bézierjeve PH krivulje s pripadajočimi števili $\mathbf{w}_k$.}
    ...
    \begin{figure}[h!]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
        \subfloat[segment]{\includegraphics[width = \groupimagewidth]{images/bezeg/prikaz-rezultata-skrcitev}}
        \qquad
...
class JXGPHBezierCurve extends JXGPointControlledCurve {
 ...
 ...
    ...
    \section{Zaključek}
    % in skripte !!! ce se le da, dej se tisto navezi
