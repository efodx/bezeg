...
%
    ...
    Enačbe iz prejšnjega razdelka lahko dobimo tudi drugače.
    Da bi zlepek bil $C^{\infty}$, bi morali deli zlepka biti deli iste Bezierjeve krivulje.
    Da bo zlepek $C^r$ pa se morata dela krivulje ujemati v $r+1$ točkah.
    To dosežemo tako, da uporabimo de Casteljaujev algoritem pri parametru $t_0$ tako, ki je izbran, da velja $\frac{t_0-1}{1}=\frac{\Delta u_{i+1}}{\Delta u_{i}}$ in preberemo krivuljo $B_{t_0+}$ iz podrazdelka~\ref{subsec:ekstrapolacija}.
    Prvih $r+1$ točk je fiksiramo, ostale pa lahko izberemo poljubno.
    Tako dobimo algoritem za enostransko konstrukcijo $C^r$ zlepka.
    \begin{algorithm}[H]
        ...
        \begin{algorithmic}
            ...
            \For{$j = 2,3\dots$st\_krivulj}
                ...
                \State $\mathbf{l}$ = decasteljau\_shema($\p^{(j-1)}$, $t_0$)
                % return decasteljau[lenn - 1][lenn - j - 1].X();
                \State $\{\p_i^{(j)}\}_{i=0}^{r}$ = $\{\mathbf{l^{(n)}_{j}}\}_{j=n}^{n-r}$
                \For{$k = 0,1,\dots (n-r)$}
        ...
        \end{algorithmic}
    \end{algorithm}
    \noindent Takšen algoritem nam ne daje lokalnosti, saj premik prvih nekaj kontrolnih točk vpliva na celotno krivuljo.
    V praksi zato ni uporaben, uporaben je le za prikazovanje lastnosti zlepka.
    V praksi potrebujemo lokalnost, zato zlepke konstruiramo simetrično.
    Podajmo algoritma za simetrični konstrukciji kvadratnega $C^1$ in kubičnega $C^2$ zlepka.
    \begin{algorithm}[H]
    ...
    \begin{algorithm}[H]
        ...
        \end{algorithmic}
    \end{algorithm}
    Primer kvadratičnega $C^1$ zlepka, konstruiranega po zgornjem algoritmu si lahko ogledamo na sliki ~\ref{};
    \begin{figure}[h]
        \centering
        \includegraphics[width = 0.40\textwidth]{images/kvadraticni-c1}
        \caption{Simetrično konstruirani kvadratični $C^1$ zlepek.}
        \label{fig:kvadraticni-c1}
    \end{figure}
    \begin{figure}[h]
        \centering
        \includegraphics[width = 0.40\textwidth]{images/kubicni-c2}
        \caption{Simetrično konstruirani kubični $C^2$ zlepek.}
        \label{fig:kubicni-c2}
    \end{figure}


    \subsubsection{Alfa parametrizacije}
    Točke $u_i$ v praksi dostikrat ne dobimo podane, ampak jih izračunamo sami.
    En izmed načinov za izbiro $u_i$ so t.i.~ \textit{alfa parametrizacije}, kjer točke $u_i$ izberemo tako, da velja
    \[\Delta u_i=|\Delta p_i|^{\alpha},\quad \alpha\in[0,1].\]
    Vpliv izbire alfe si lahko ogledamo na sliki.
    \begin{figure}[h]
        \centering
        \subfloat[$\beta_1=1$]{\includegraphics[width = \groupimagewidth]{images/zlepek-g1}}
        \qquad
    ...
    \begin{figure}[h]
        ...
        \caption{Vpliv izbore $\alpha$ pri izračunu točk $u_i$.}
        \label{fig:zlepek-g1}
    \end{figure}
%
...
%    V poglavju se bomo osredotočili (mogoce celo samo govorili?) o krivuljah v prostoru $\R^2$.
    ...
    \end{align} % ZA KATERE SISTEME JE TO IDEALNO?!
    Idealno bi bilo, da je parametrična hitrost $\sigma \equiv 1$.
...
%    \vspace{-0.2cm}
    ...
    PH krivulja, pridobljena z integriranjem polinomov $x'$ in $y'$ iz~\ref{eq:hodograf-splosni}, je stopnje $n=\lambda + 2\mu + 1$.
...
%        Ostane nam $n-2$ prostih parametrov za obliko krivulje.}
    ...
    Krivulje bomo konstruirali s pomočjo izreka~\ref{eq:hodograf-splosni}, kjer za polinomom $w$ vzamemo $w\equiv 1$.
