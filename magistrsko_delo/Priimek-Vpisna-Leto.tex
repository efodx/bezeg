...
\begin{document}
    ...
    \begin{definicija}
        Za pozitivno naravno število $n$ je $i$-ti \textit{Bernsteinov bazni polinom} enak \[B_i^n(t)\coloneqq\bernstein{n}{i}.\]
    \end{definicija}
    \noindent Linearni kombinaciji takšnih polinomov t.j.\ $\bsum$, pravimo \textit{Bernsteinov polinom} stopnje $n$.
    V izreku~\ref{izrek:bernsteinovi_lastnosti} naštejemo nekaj lastnosti Bernsteinovih baznih polinomov, ki so ključne za rabo v CAD sistemih\mycomment{(zato da so bez krivulje sploh uporabne).}
%    Takšni polinomi so bili prvič uporabljeni v konstruktivnem(????) dokazu Weierstrassovega aproksimacijskega izreka.
    ...
    \begin{izrek}{Lastnosti Bernsteinovih polinomov}
        ...
        \begin{enumerate}
            ...
            \item $B_i^n(t) = (1-t)B_i^{n-1} + tB_{i-1}^{n-1}$ \label{izrek:bernsteinovi_lastnosti:rekruzija}
            \item $B_{i}^{\prime n}(t)=n(B_{i-1}^{n-1} - B_{i}^{n-1})$ \label{izrek:bernsteinovi_lastnosti:odvod}
    ...
    \end{definicija}
    Točkam $\p_i$ pravimo \textit{kontrolne točke}, poligonu, ki ga dobimo, če povežemo točke $\p_i$ in $\p_{i+1}$ za $i=0,\dots,n-1$, ter točki $\p_0$ in $\p_n$ pa pravimo \textit{kontrolni poligon}.
    \mycomment{Opomba: pri slikovnem prikazovanju raznih konceptov Bezierjevih krivulj, se bomo omejili na $d=2$, torej na Bezierjeve krivulje v ravnini.}
    ...
    Izkaže se, da imajo uteži racionalne Bezierjeve krivulje geometrijski pomeni, ki ga lahko uporabimo zgornje.
    ...
    V tem podpodrazdelku bomo izpeljali takšne kontrolne točke in uteži racionalne Bezierjeve krivulje, da bo krivulja predstavljala izsek krožnice s kotom $\alpha$, polmerom $r$ in središčem $\mathbf{0}=(0,0)$.
    Izsek krožnice bomo predstavljali s kvadratičnimi racionalnimi Bezierjevimi krivuljami.
    Za konstrukcijo imamo na voljo kontrolne točke $\p_0,\p_1$ in $\p_2$.
    Racionalne Bezierjeve krivulje interpolirajo končne točke, zato morata točki $\p_0$ in $\p_2$ ležati na začetku ter koncu krožnega izseka.
    Ker so racionalne Bezierjeve krivulje afino invariantne, se lahko osredotočimo na primer, kjer točka $\p_1$ leži na abscisi.
    Da bo temu tako, mora veljati $\p_0=(r,0)$ in $\p_2=( r\cos(\alpha),r\sin(\alpha))$.
    Iz izreka~\ref{izrek:racionalne-utezi-1} sledi, da lahko fiksiramo robni uteži $w_0=w_2=1$.
    Ker je krožni izsek simetričen čez premico z naklonom $\frac{\alpha}{2}$, krivulja pa parabola, lahko pričakujemo, da bosta točki $\p_0$ in $\p_1$ ležali na premici med točkama $\p_0$ in $\p_2$ t.j.\ $\p_1=\left(R\cos\left(\frac{\alpha}{2}\right),R\sin\left(\frac{\alpha}{2}\right)\right)$.
    Sedaj je cilj najti vrednosti $R$ in $w_1$.
    Najprej zapišimo parametrizacijo željene krivulje
    ...
    Po komponentah to izgleda takole
    \begin{align*}
        x(t) &= \frac{r (1-t)^2 + w_1R\cos\left(\frac{\alpha}{2}\right) 2t(1-t) + r\cos(\alpha)t^2}{(1-t)^2 + w_1 2t(1-t) + t^2} \\
        y(t) &= \frac{w_12t(1-t)R\sin\left(\frac{\alpha}{2}\right) + r\sin(\alpha)t^2}{(1-t)^2 + w_1 2t(1-t) + t^2}.
    \end{align*}
    Zaradi simetrije mora krivulja v $t=\frac{1}{2}$ interpolirati točko $\left(r\cos\left(\frac{\alpha}{2}\right),r\sin\left(\frac{\alpha}{2}\right)\right)$ iz česar sledi
    \begin{align*}
        x\left(\frac{1}{2}\right) &= \frac{r \frac{1}{4} + w_1R\cos\left(\frac{\alpha}{2}\right) \frac{1}{2} + r\cos(\alpha)\frac{1}{4}}{\frac{1}{4}+ w_1\frac{1}{2} + \frac{1}{4}} = \frac{ \frac{r}{2} + w_1R\cos\left(\frac{\alpha}{2}\right) + \frac{r}{2}\cos(\alpha)}{1+w_1} = r\cos\left(\frac{\alpha}{2}\right) \\
        y\left(\frac{1}{2}\right) &= \frac{\frac{w_1}{2}R\sin\left(\frac{\alpha}{2}\right) + \frac{r}{4}\sin(\alpha)}{\frac{1}{4}+ w_1\frac{1}{2} + \frac{1}{4}} =  \frac{w_1R\sin\left(\frac{\alpha}{2}\right) + \frac{r}{2}\sin(\alpha)}{1+w_1} = r\sin\left(\frac{\alpha}{2}\right).
    \end{align*}
    NEXT
    \begin{align*}
        \frac{r}{2} + w_1R\cos\left(\frac{\alpha}{2}\right) + \frac{r}{2}\cos(\alpha) &= (1+w_1)r\cos\left(\frac{\alpha}{2}\right) \\
        w_1R\cos\left(\frac{\alpha}{2}\right) - w_1r\cos\left(\frac{\alpha}{2}\right) &= r\cos\left(\frac{\alpha}{2}\right)-\frac{r}{2} - \frac{r}{2}\cos(\alpha) \\
        w_1(R-r)\cos\left(\frac{\alpha}{2}\right) &= r\left(\cos\left(\frac{\alpha}{2}\right)-\frac{1}{2} - \frac{1}{2}\cos(\alpha)\right)
    \end{align*}
    NEXT
    \begin{align*}
    ...
    Tangenta na krožnico je v točki $(r,0)$ vzporedna z ordinatno osjo, zato mora veljati $x'(0)=0$.
    Odvod krivulje v točki $(r,0)$ je enak $2(\p_{1}-\p_{0}) = 2\left(R\sin\left(\frac{\alpha}{2}\right)-r,R\cos\left(\frac{\alpha}{2}\right)\right)$.
