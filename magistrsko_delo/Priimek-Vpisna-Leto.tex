...
\begin{document}
    ...
    Nadaljevali pa bomo z nekaj lastnostmi Bezierjevih krivuljah, ki so ključne za njihovo rabo v CAGD sistemih.
    ...
    V tem podrazdelku bomo obnovili znanje o Bernsteinovih polinomih.
    Začnimo z definicijo Bernsteinovih baznih polinomov.
    % nekaj njihovih osnovnih lastnosti, ki jih bomo uporabili pri Bezierjevih krivuljah.
    % Za začetek jih definirajmo.
    \begin{definicija}
        \label{def:bernstein}
        Za pozitivno naravno število $n$ je $i$-ti \textit{Bernsteinov bazni polinom} podan s predpisom \[B_i^n(t)\coloneqq\bernstein{n}{i}.\]
    \end{definicija}
    ...
    Takšni bazi pravimo \textit{Bernsteinova baza} stopnje $n$, polinomu izraženemu v njej pa pravimo \textit{Bernsteinov polinom}.
    \begin{primer}
        ...
        Za stopnjo $n=1$ dobimo polinoma s funkcijskimi predpisi $B_{0}^{1}(t) = 1-t$ in $B_{1}^{1}(t) = t$, ki sta znana iz linearne interpolacije dveh točk.
    ...
    \end{figure}
    V izreku~\ref{izrek:bernsteinovi_lastnosti} bomo našteli nekaj lastnosti Bernsteinovih baznih polinomov, ki so ključne za rabo v CAGD sistemih.
    \begin{izrek}
...
%    \end{dokaz}
    ...
    V naslednjem izreku podajmo rekurzivno zvezo za računanje vrednosti Bernsteinovih baznih polinomov.
    \begin{izrek}
        \label{izrek:bernsteinovi_lastnosti:rekruzija}
        Za Bernsteinove bazne polinome velja rekurzivna zveza
        \[B_i^n(t) = (1-t)B_i^{n-1}(t) + tB_{i-1}^{n-1}(t),\ \forall t\in\R.\]
    \end{izrek}
%    Zvezo tudi dokažimo.
%    \begin{dokaz}
%        Desni del rekurzivne zveze manipuliramo, da pridemo do levega
%        \begin{align*}
%        (1-t) B_i^{n-1}(t) + tB_{i-1}^{n-1}(t) &= \binom{n-1}{i}t^{i}(1-t)^{n-i} + \binom{n-1}{i-1}t^{i}(1-t)^{n-i}  \\
...
%    \end{dokaz}
    ...
    Ker Bernsteinovi bazni polinomi stopnje $n$ tvorijo bazo prostora $\mathbb{P}_n$, lahko njihove odvode v Bernsteinovi bazi stopnje $n-1$.
...
%    \end{dokaz}
    Za konec podrazdelka podajmo še formulo za zmnožek dveh Bernsteinovih polinomov.
    \begin{izrek}
        %\item Njihov integral pa zvezi $\int B_i^n(t)dt= \frac{1}{n+1}\sum^{n+1}_{j=i+1}B_{j}^{n+1}(t) + C, C\in\R$ \label{izrek:bernsteinovi_lastnosti:integral}
    ...
    \end{izrek}
%    \begin{dokaz}
%        Naj bosta $f(t)=\sum_{i=0}^{m}\alpha_iB_i^n(t)$ in $g(t)\sum_{i=0}^{n}\beta_iB_i^n(t)$ Bernsteinova polinoma.
%        \begin{align*}
...
%    \end{dokaz}
    ...
    Z željo po krajših in bolj preglednih zapisih bomo uvedli večdimenzionalne oznake.
    ...
    Če v Bernsteinov polinom $n$-te stopnje namesto skalarjev vstavimo točke $\p_i\in\R^d$, za $i=0,\ldots,n$, dobimo predpis parametrizacije Bezierjeve krivulje.
    \begin{definicija}
    ...
    \end{definicija}
    \begin{opomba}
        Pri slikovnem gradivu raznih konceptov Bezierjevih krivulj se bomo omejili na stopnjo $d=2$, torej na Bezierjeve krivulje v ravnini.
    \end{opomba}
    Na sliki~\ref{fig:bezierjeva-krivulja} si lahko ogledamo primere Bezierjevih krivulj in njihovih kontrolnih poligonov za $n=1,2,3,4$.
    Zapišimo sedaj nekaj njihovih lastnosti.
    \begin{figure}[ht!]
        \captionsetup[subfigure]{labelformat=empty}
        \centering
        ...
        \caption{Bezierjeve krivulje s pripadajočimi kontrolnimi poligoni za $n=1,2,3,4$}
    ...
    Zaradi Afine invariance lahko uporabnikove transformacije krivulje v ozadju prevedemo v transformacije kontrolnih točk.
    ...
    Lastnosti si sedaj oglejmo tudi na slikah.
...
%    \end{dokaz}
    ...
    Direktno računanje vrednosti Bernsteinovih polinomov preko enačb iz definicije~\ref{def:bernstein} pa ni stabilno.
    Da bomo lahko točke Bezierjevih krivulj računali stabilno potrebujemo sledeč izrek.
    \begin{izrek}
        ...
        Potem za poljubno realno število $t$ velja rekurzivna zveza \[\bigbbt_{[\p_0,\p_1,\dots,\p_n]} = (1-t)\bigbbt_{[\p_0,\p_1,\dots,\p_{n-1}]} +t\bigbbt_{[\p_1,\p_2,\dots,\p_n]}.\]
    \end{izrek}
    Izrek tudi dokažimo.
    \begin{dokaz}
        Desni del rekurzivne zveze iz izreka razpišemo in računamo, da dobimo
        \begin{align*}
        (1-t)
            &
            \bigbbt_{[\p_0,\p_1,\dots,\p_{n-1}]}+t\bigbbt_{[\p_1,\p_2,\dots,\p_n]} = \\
            &= (1-t)\bernsteinsump{i}{n-1}+t\sum_{i=0}^{n-1} \p_{i+1}B_i^{n-1}(t) \\
    ...
    \begin{figure}
        ...
        \end{tabular}
        \caption{Decasteljaujeva shema \mycomment{Menjal bom sliko, ko zadevo implementiram na Bezgu}}
    \end{figure}
    ...
    \noindent    Algoritem ima tudi geometrijski pomen.
    Pri stopnji $n=1$ se namreč prevede na interpolacijo dveh točk, kar lahko vidimo na sliki~\ref{fig:dec-alg-n1}.
...
%    \end{figure}
    ...
    V CAGD in CAD sistemih se mnogokrat zgodi, da uporabnik želeli ohraniti le en del Bezierjeve krivulje.
    Naj bo to del krivulje, ki ga dobimo tako, da za prvotno krivuljo parameter $t$ omejimo na interval $[0,t_0]$ za neko pozitivno realno število $t_0<1$.
...
%    \end{dokaz}
    \begin{figure}[h!]
        \centering
        \includegraphics[width=0.8\textwidth]{images/subidivizja}
        \caption{Subdivizija}
    ...
    \begin{figure}[h]
        \centering
        \subfloat[začetna krivulja]{\includegraphics[width = 0.25\textwidth]{images/dvig-prej}}
        \qquad
        \subfloat[eno višanje]{\includegraphics[width = 0.25\textwidth]{images/dvig-po}}
        \qquad
        \subfloat[20 višanj]{\includegraphics[width = 0.25\textwidth]{images/dvig-n}}
        \caption{Višanje Bezierjeve krivulje}
        \label{fig:višanje}
    \end{figure}
    \begin{figure}[h]
        \centering
        \subfloat[20 višanj]{\includegraphics[width = 0.45\textwidth]{images/dvig-n-poligon}}
        \qquad
        \subfloat[35 višanj]{\includegraphics[width = 0.45\textwidth]{images/dvig-n2-poligon}}
    ...
    Farinove točke ležijo na daljicah kontrolnega poligona in jih je toliko, kolikor je prostih parametrov uteži, tj.\ $n-1$.
