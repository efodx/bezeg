...
class DecasteljauGraph extends BaseBezierCurveGraph<any, DecasteljauGraphStates> {
    ...
    private prevtime: DOMHighResTimeStamp | undefined;
    ...
    private animate(animate: boolean) {
        if (animate) {
            this.animating = true;
            this.animateRecursive(this.state.t, undefined as any);
        } else {
            this.animating = false;
        }
    }

    private animateRecursive(t: number, time: DOMHighResTimeStamp) {
        if (this.prevtime && time) {
            const delta = time - this.prevtime;
            let dt = 0.015 * this.state.v * delta / 100 * 2;
            t = t + dt;
            if (t > 1) {
                t = 0;
            }
        ...
        this.prevtime = time;
        if (this.animating) {
            window.requestAnimationFrame((time) => this.animateRecursive(t, time));
        }
    }
